...
def rotate(hr, n):
    if len(list(hr.size())) == 5:
        hr = hr[:, 0, :, :, :]

    hr_s = hr
    if n > 1:
        list_n = random.sample(range(1, 16), n - 1)
        hr_ = torch.cat((hr[list_n[0]:, :, :, :], hr[:list_n[0], :, :, :]), dim=0)
        hr_s = torch.stack((hr, hr_), dim=1)

        for i in range(n - 2):
            hr_ = torch.cat((hr[list_n[i + 1]:, :, :, :], hr[:list_n[i + 1], :, :, :]), dim=0)
            hr_s = torch.cat((hr_s, hr_.unsqueeze(1)), dim=1)
...
class checkpoint():
    ...
    def write_log(self, log, local_rank=0, refresh=False):
        print(log)
        if local_rank == 0:
