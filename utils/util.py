...
from utils.jpeg_utils import DiffJPEG
...
class complex_SRMDPreprocessing(object):
    def __init__(self,
                 scale,
                 mode='bicubic',
                 kernel_size=21,
        ...
        self.jpeg = DiffJPEG(differentiable=False).cuda()
    ...
    def __call__(self, hr_tensor, random=True):
        with torch.no_grad():
            if self.train:
                ...
                origin = hr_tensor
                b_kernels = self.gen_kernel(B, True)  # B degradations

                hr_blured = self.blur(hr_tensor.view(B, -1, H, W), b_kernels).view(B, N, C, H, W)
                hr_blured[gate] = origin[gate]
                ...
                gate = (torch.rand(B) > 0.5)
                origin = lr_blured.view(B, N, C, H//int(self.scale), W//int(self.scale)).clone()

                noise_level = torch.rand(B, 1, 1, 1, 1).to(lr_blured.device) * (self.noise_max-self.noise_min) + self.noise_min
                noise = torch.randn_like(lr_blured).view(-1, N, C, H_lr, W_lr).mul_(noise_level).view(-1, C, H_lr, W_lr)
                lr_blured.add_(noise)

                lr_blured = lr_blured.view(B, N, C, H//int(self.scale), W//int(self.scale))
                lr_blured[gate] = origin[gate]
                lr_blured = torch.clamp(lr_blured / 255., 0, 1)  # clamp to [0, 1], otherwise JPEGer will result in unpleasant artifacts

                # JPEG
                gate = (torch.rand(B) > 0.5)
                origin = lr_blured.clone()
                quality = torch.rand(B).to(lr_blured.device) * (self.jpeg_max-self.jpeg_min) + self.jpeg_min
                for i in range(lr_blured.size(1)):
                    lr_blured[:, i, ...] = self.jpeg(lr_blured[:, i, ...], quality=quality.clone())
                lr_blured[gate] = origin[gate]

                lr_blured = torch.clamp((lr_blured * 255.0).round(), 0, 255) / 255.
            else:
                ...
                # gaussian blur + downsampling
                    ...
                    b_kernels = self.gen_kernel(B, False)  # B degradations
                    # blur
                ...
                if self.noise > 0:
                    ...
                    noise = torch.randn_like(lr_blured).mul_(noise_level)
                    lr_blured.add_(noise)

                lr_blured = torch.clamp(lr_blured / 255., 0, 1)
                # JPEG
                if self.jpeg_factor > 0:
                    lr_blured = self.jpeg(lr_blured, quality=self.jpeg_factor)
                    lr_blured = torch.clamp((lr_blured * 255.0).round(), 0, 255) / 255.
...
class SRMDPreprocessing(object):
    ...
    def __call__(self, hr_tensor, random=True):
        with torch.no_grad():
            ...
            return lr_blured.view(B, N, C, H//int(self.scale), W//int(self.scale))
